export const ja = {
  ...
  consentSection1Content: '・ 研究の目的：視覚的表現（ノードリンク図）の新規設計・改善によりデータの有効活用を促進するための知見を収集します。\n・ 実験の目的：異なる表示方法（2D表示、3D表示、立体視の有無、操作の自由度など）によるノードリンク図の可読性への影響を検証します。\n・ 実験の方法：画面に表示されたノードリンク図（点と線で構成される図形）を見て、最短経路の距離判定や共通隣接ノードの選択などのタスクに答えてもらいます。表示方法は2D表示、3D表示（固定視点・立体視なし）、3D表示（固定視点・立体視あり）、3D表示（自由視点・立体視なし）など様々な条件で提示されます。\n・ 対象の実験参加者：18歳以上の方を対象としています。\n・ 研究期間：本研究は2027年3月31日までの実施を予定しています。',
