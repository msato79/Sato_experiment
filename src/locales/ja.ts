export const ja = {
  ...
  // Task instructions
  taskATitle: 'タスクA: 最短経路距離の判定',
  taskAInstruction: '2つのハイライトされたノードの間を、エッジでつないでいくとき、最短で何本のエッジを通ればよいかを判定してください。\n答えは「距離 2」（エッジを2本通る）か「距離 3以上」（エッジを3本以上通る）のどちらかです。「距離 2」または「距離 3以上」のボタンをクリックして回答してください。',
  ...
  // Instruction screens
  instructionTaskA: 'これから最短経路の距離を判定するタスクを行います。\n\n【ノードリンク図について】\nこの実験では「ノードリンク図」を使います。ノードリンク図は以下の要素でできています：\n・ノード（点）：丸い点で表されます\n・エッジ（線）：2つの点を結ぶ線です\n・経路：いくつかのエッジをたどって、あるノードから別のノードへ移動する道筋です\n・距離：ある経路を構成するエッジの本数です\n\n【タスクの内容】\n画面上で2つのノードがハイライトされます。この2つのノード間を、最短経路で移動するときに通るエッジの本数が2か、それとも3以上かを判定してください。「距離 2」または「距離 3以上」のボタンをクリックして回答してください。\n\n【実験の流れ】\n1. まず練習問題を2問行います。練習では解答後に正解が表示されます。\n2. 練習が終わったら、本番問題を12問行います。\n\n【表示方法について】\nグラフは様々な表示方法で提示されます：\n・2D表示（平面表示）\n・3D表示（固定視点）\n・3D表示（小さい回転）\n・3D表示（大きい回転）\n・軸オフセット\n\n各問題で異なる表示方法が使われます。',
  ...
  consentFlowStep6: 'タスクBの説明',
  consentFlowStep7: 'タスクBの練習（2問）',
  consentFlowStep8: 'タスクBの本番（12問）',
