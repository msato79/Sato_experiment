export const ja = {
  ...
  participantIdBackButton: '戻る',
  
  // Task instructions
  taskATitle: 'タスクA: 最短経路のエッジ数の判定',
  taskAInstruction: '2つのハイライトされたノードの間を、エッジでつないでいくとき、最短で何本のエッジをたどればよいかを判定してください。\n答えは「エッジ2本」（エッジを2本たどる）か「エッジ3本以上」（エッジを3本以上たどる）のどちらかです。「エッジ2本」または「エッジ3本以上」のボタンをクリックして回答してください。',
  ...
  // Instruction screens
  instructionTaskA: 'これから最短経路のエッジ数を判定するタスクを行います。\n\n【ノードリンク図について】\nこの実験では「ノードリンク図」を使います。ノードリンク図は以下の要素でできています：\n・ノード（点）：丸い点で表されます\n・エッジ（線）：2つの点を結ぶ線です\n\n【タスクの内容】\n画面上で2つのノードがハイライトされます。この2つのノード間を、最短経路でたどるときにたどるエッジの本数が2か、それとも3以上かを判定してください。\nここで、「経路」とはいくつかのエッジをたどって、あるノードから別のノードへたどる道筋のことです。「最短経路」とは、そのような経路のうち、たどるエッジの本数が最も少ない経路のことです。「エッジ2本」または「エッジ3本以上」のボタンをクリックして回答してください。\nできるだけ早く、かつ正確に回答してください。\n\n【実験の流れ】\n1. まず練習問題を2問行います。練習では回答後に正解が表示されます。\n2. 練習が終わったら、本番問題を12問行います。回答と回答時間を記録します。\n\n【表示方法について】\n各問題で条件A、条件B、条件C、条件Dのいずれかが使われます。\n各問題が始まる前に3秒のカウントダウンが表示されます。このカウントダウン中に、その問題の表示方法を確認してください。',
  instructionTaskB: 'これから共通隣接ノードを選択するタスクを行います。\n\n【ノードリンク図について】\nこの実験では「ノードリンク図」を使います。ノードリンク図は以下の要素でできています：\n・ノード（点）：丸い点で表されます\n・エッジ（線）：2つのノードを結ぶエッジです\n・隣接：あるノードと別のノードが、直接1本のエッジでつながっているとき、それらのノードは「隣接している」といいます\n\n【タスクの内容】\n画面上で2つのノードがハイライトされます。この2つのノードの両方に隣接しているノードをすべて選択してください。共通隣接ノードがない場合もあります。ノードをクリックして選択し、すべて選択したら「次へ進む」をクリックしてください。\nできるだけ早く、かつ正確に回答してください。\n\n【実験の流れ】\n1. まず練習問題を2問行います。練習では回答後に正解が表示されます。\n2. 練習が終わったら、本番問題を12問行います。回答と回答時間を記録します。\n\n【表示方法について】\n各問題で条件A、条件B、条件C、条件Dのいずれかが使われます。\n各問題が始まる前に3秒のカウントダウンが表示されます。このカウントダウン中に、その問題の表示方法を確認してください。',
  ...
  consentSection1Content: '・ 研究の目的：視覚的表現（ノードリンク図）の新規設計・改善によりデータの有効活用を促進するための知見を収集します。\n・ 実験の目的：異なる表示方法によるノードリンク図の可読性への影響を検証します。\n・ 実験の方法：画面に表示されたノードリンク図（点と線で構成される図形）を見て、最短経路の距離判定や共通隣接ノードの選択などのタスクに答えてもらいます。表示方法は条件A、条件B、条件C、条件Dなど様々な条件で提示されます。\n・ 対象の実験参加者：18歳以上の方を対象としています。\n・ 研究期間：本研究は2027年3月31日までの実施を予定しています。',
  ...
  consentFlowStep1: '同意書の確認と同意（約2分）',
  consentFlowStep2: '参加者IDの入力（約1分）',
  consentFlowStep3: 'タスクAの説明（約2分）',
  consentFlowStep4: 'タスクAの練習（2問）（約2分）',
  consentFlowStep5: 'タスクAの本番（12問）（約6分）',
  consentFlowStep6: 'タスクAについてのアンケート（約2分）',
  consentFlowStep7: 'タスクBの説明（約2分）',
  consentFlowStep8: 'タスクBの練習（2問）（約2分）',
  consentFlowStep9: 'タスクBの本番（12問）（約9分）',
  consentFlowStep10: 'タスクBについてのアンケート（約2分）',
  consentEstimatedTime: '所要時間',
  consentTimeValue: '約30分',
  consentImportantNotes: '実験を行うにあたってのお願い',
  consentNote1: '全画面を推奨しますが、ディスプレイサイズが大きい場合は適宜見やすいウィンドウサイズでこの実験を行なってください。',
